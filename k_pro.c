...
void send_with_length(int socket, const char *msg) {
    int len = htonl(strlen(msg));  // Konwersja długości do big-endian (network byte order)
...
int recv_with_length(int socket, char *buffer, int buffer_size) {
    ...
    buffer[received] = '\0';  // Dodanie null-terminatora dla bezpieczeństwa
    return received;  // Zwrócenie liczby odebranych bajtów
}
